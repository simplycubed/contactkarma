...
func (o *DeleteContactNoteURL) Build() (*url.URL, error) {
	...
	var _path = "/unified/{unified_id}/notes/{note_id}"

	noteID := o.NoteID
	if noteID != "" {
		_path = strings.Replace(_path, "{note_id}", noteID, -1)
	} else {
		return nil, errors.New("noteId is required on DeleteContactNoteURL")
	}
	...
	if unifiedID != "" {
		...
		return nil, errors.New("unifiedId is required on DeleteContactNoteURL")
	}
...
func (o *DeleteContactNoteURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on DeleteContactNoteURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on DeleteContactNoteURL")
	}
