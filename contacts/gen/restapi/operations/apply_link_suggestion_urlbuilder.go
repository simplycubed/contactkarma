...
func (o *ApplyLinkSuggestionURL) Build() (*url.URL, error) {
	...
	var _path = "/suggestions/{suggestion_id}/apply"

	suggestionID := o.SuggestionID
	if suggestionID != "" {
		_path = strings.Replace(_path, "{suggestion_id}", suggestionID, -1)
	} else {
		return nil, errors.New("suggestionId is required on ApplyLinkSuggestionURL")
	}
...
func (o *ApplyLinkSuggestionURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on ApplyLinkSuggestionURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on ApplyLinkSuggestionURL")
	}
