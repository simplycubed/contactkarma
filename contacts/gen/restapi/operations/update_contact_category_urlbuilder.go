...
func (o *UpdateContactCategoryURL) Build() (*url.URL, error) {
	...
	var _path = "/unified/{unified_id}/category"

	unifiedID := o.UnifiedID
	if unifiedID != "" {
		_path = strings.Replace(_path, "{unified_id}", unifiedID, -1)
	} else {
		return nil, errors.New("unifiedId is required on UpdateContactCategoryURL")
	}
...
func (o *UpdateContactCategoryURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on UpdateContactCategoryURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on UpdateContactCategoryURL")
	}
