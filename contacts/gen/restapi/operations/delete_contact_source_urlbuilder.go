...
func (o *DeleteContactSourceURL) Build() (*url.URL, error) {
	...
	var _path = "/contacts/sources/{source_id}"

	sourceID := o.SourceID
	if sourceID != "" {
		_path = strings.Replace(_path, "{source_id}", sourceID, -1)
	} else {
		return nil, errors.New("sourceId is required on DeleteContactSourceURL")
	}
...
func (o *DeleteContactSourceURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on DeleteContactSourceURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on DeleteContactSourceURL")
	}
